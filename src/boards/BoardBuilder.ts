...
export class BoardBuilder {
  ...
  // "Able am I, Son."
    ...
    getBestNotPlant(rng: Random): number {
    ...
    getBestNotOcean(rng: Random): number {
      let candidates: Array<number> = [];
    ...
    public shuffle(rng: Random, ...lands: Array<SpaceName>) {
      ...
      for (const i of idOrder) {
        ...
        if (oceanDegree === 1 && rng.nextInt(2) >= 1) continue;
        // 1.0 if adjacent oceans count ==0
        const bestNotOcean = this.getBestNotOcean(rng);
      ...
      for (const i of idOrder) {
        ...
        if (plantDegree === 1 && rng.nextInt(2) >= 1) continue;
        // 1.0 if adjacent plants count ==0
        const bestNotPlant = this.getBestNotPlant(rng);
